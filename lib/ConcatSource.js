...
class ConcatSource extends Source {
	...
	async streamTo(writableStream) {
		...
		// Helper to write a buffer to stream without ending it
			...
			return new Promise((resolve, reject) => {
				...
				const write = () => {
					while (offset < buffer.length) {
						const chunk = buffer.slice(offset, offset + 65536);
		...
		// Stream each child sequentially without concatenating in memory
		for (const child of /** @type {Source[]} */ (this._children)) {
			// Get buffer from child without calling streamTo (which would end the stream)
			let buffer;
			if (typeof child.buffer === "function") {
				buffer = child.buffer();
			} else {
				const source = child.source();
				buffer = Buffer.isBuffer(source) ? source : Buffer.from(source, "utf8");
			}
			
			await writeBuffer(buffer);
		}
		
		// End the stream after all children are written
		return new Promise((resolve, reject) => {
			writableStream.on("error", reject);
