		...
		const writeNextChunk = () => {
			if (currentBuffer === null) {
				...
				if (childIndex >= children.length) {
					// All children written, close stream
					writer
						.close()
						.then(() => {
							writer.releaseLock();
				...
				if (typeof child.buffer === "function") {
					currentBuffer = child.buffer();
				} else {
					const source = child.source();
					currentBuffer = Buffer.isBuffer(source)
						? source
			...
			if (currentOffset >= currentBuffer.length) {
				// Current buffer done, move to next
				currentBuffer = null;
				currentOffset = 0;
				writeNextChunk();
				return;
			...
			const chunk = currentBuffer.slice(
				currentOffset,
				currentOffset + 65536,
			); // 64KB chunks
